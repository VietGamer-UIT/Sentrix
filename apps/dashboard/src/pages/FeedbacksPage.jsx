...
import { MOCK_FEEDBACKS, timeAgo } from '../mocks/firestoreMock.js'
...
const LOCATIONS = [...new Set(MOCK_FEEDBACKS.map(f => f.location))]
 ...
 * Đọc: tenants/{tenant_id}/feedbacks orderBy timestamp DESC
 */
export default function FeedbacksPage() {
  ...
  const filtered = useMemo(() => {
    return [...MOCK_FEEDBACKS]
      ...
      .filter(f => {
        if (filterSentiment === 'positive' && f.sentiment_score < 0.3) return false
        if (filterSentiment === 'negative' && f.sentiment_score > -0.3) return false
        if (filterSentiment === 'neutral' && (f.sentiment_score <= -0.3 || f.sentiment_score >= 0.3)) return false
  ...
  }, [filterSentiment, filterLocation, filterType, search])
        ...
        <div className="table-wrapper">
          <table>
            ...
            <tbody>
              ...
              {filtered.map(fb => {
                const isPos = fb.sentiment_score >= 0.3
                const isNeg = fb.sentiment_score <= -0.3
                ...
                return (
                  <tr key={fb.feedback_id}>
                    ...
                    <td style={{ fontSize: 'var(--font-size-lg)' }}>
                      {fb.input_type === 'audio' ? '🎙️' : '✍️'}
                    </td>
                    <td style={{ maxWidth: 280 }}>
                      {fb.processing_status === 'processing' ? (
                        <span style={{ color: 'var(--color-text-muted)', fontStyle: 'italic', fontSize: 'var(--font-size-xs)' }}>
                          ⏳ Đang phân tích...
                        </span>
                      ) : (
                    ...
                    <td style={{ textAlign: 'center' }}>
                      {fb.processing_status === 'done' ? (
                        <span className={`sentiment-badge sentiment-badge--${sentCls}`}>
                          {isPos ? '+' : ''}{fb.sentiment_score.toFixed(2)}
                        </span>
                      ) : '—'}
                    </td>
                    <td style={{ maxWidth: 180 }}>
                      ...
                      {fb.aspects.length > 3 && <span className="aspect-chip">+{fb.aspects.length - 3}</span>}
                    </td>
                    <td>
                      <span className={`status-badge--${fb.processing_status}`} style={{ fontSize: 'var(--font-size-xs)', fontWeight: 600 }}>
                        {fb.processing_status === 'done' ? '✅ Xong'
                          : fb.processing_status === 'processing' ? '⏳ Xử lý'
                          : fb.processing_status === 'error' ? '❌ Lỗi'
                          : '⏸️ Chờ'}
                      </span>
                    </td>
                  </tr>
